# Intel 8086 ile Mikroişlemci Programlamaya Giriş

**Sürüm 0.0.1 — 2026**

> ℹ️ **Bilgi:** Bu kitap yeni hazırlanmış olup içerik düzenli olarak güncellenmektedir. Yorum ve önerileriniz için: oergin@sharjah.ac.ae

**Yazarlar:** Prof. Dr. Oğuz Ergin ve Şadi Çağatay Öztürk

**GitHub Pages:** https://prof-oguzergin.github.io/mikroislemciler/

## Kitap Hakkında

Bu kitap, Intel 8086 mikroişlemcisi üzerinden çevirici dili (assembly language) programlamayı öğretmek amacıyla hazırlanmıştır. Kitap, bilgisayar mühendisliği ve elektrik-elektronik mühendisliği öğrencilerine yönelik olup mikroişlemci mimarisi, sayı sistemleri, buyruk kümesi ve sistem programlama konularını kapsamaktadır.

## İçindekiler

| Bölüm | Konu |
|-------|------|
| 1 | Bilgisayar Sistemlerine Giriş |
| 2 | Sayı Sistemleri |
| 3 | 8086 Mimarisi |
| 4 | Çevirici Diline Giriş |
| 5 | Veri Taşıma Buyrukları |
| 6 | Aritmetik ve Mantık Buyrukları |
| 7 | Denetim Buyrukları |
| 8 | Dizgi İşleme Buyrukları |
| 9 | Kesmeler ve Çevre Birimlerinin Kullanımı |
| 10 | Girdi/Çıktı İşlemleri |
| 11 | Basit Bir İşletim Sistemi |
| Ek A | x86 Buyruklarının Listesi |
| Ek B | Derleme Araçları (FASM/NASM) |
| Ek C | DOS ve Linux Kesmeleri |
| Ek D | BIOS Kesmeleri |
| Ek E | ASCII Kodları ve Klavye Tarama Kodları |

## PDF

Kitabın güncel sürümüne bu depodaki [ana-kitap.pdf](ana-kitap.pdf) dosyasından erişebilirsiniz.

Sürüm notlarıyla birlikte indirmek için: https://github.com/prof-oguzergin/mikroislemciler/releases/latest

## Lisans

Bu eser **Creative Commons Atıf-GayriTicari-Türetilemez 4.0 Uluslararası (CC BY-NC-ND 4.0)** lisansı ile lisanslanmıştır. Özgürce paylaşılabilir ve dağıtılabilir; uygun atıf yapılması, ticari amaçla kullanılmaması ve değişiklik yapılmaması koşuluyla.

https://creativecommons.org/licenses/by-nc-nd/4.0/deed.tr

**ISBN:** 978-625-00-5838-1

**İletişim:** oergin@sharjah.ac.ae
